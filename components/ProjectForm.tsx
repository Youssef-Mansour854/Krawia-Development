  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 max-w-4xl font-sans">
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border pb-3">
                  <div className="flex items-center gap-3">
                    <CategoryVectorIcon iconKey={win.key} />
                    <div>
                      <h4 className="font-serif text-lg font-bold text-ink flex items-center gap-2">
                        <span>{win.title}</span>
                        {isCustomWin && (
                          <span className="text-[10px] bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-xs font-sans font-normal">
                            نافذة مخصصة
                          </span>
                        )}
                      </h4>
                  ...
                  <div className="flex items-center gap-3 self-start sm:self-auto">
                    ...
                    {isCustomWin && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 text-xs font-semibold text-red-600 hover:text-red-800 border border-red-200 hover:border-red-400 bg-red-50 hover:bg-red-100 px-2.5 py-1 transition-colors rounded-xs"
                        title="حذف هذه النافذة بجميع مستنداتها"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                        <span>حذف النافذة</span>
                      </button>
                    )}
                  </div>
                </div>
      ...
      <div className="flex items-center justify-end gap-3 border-t border-border pt-6">
        <Link
          href="/admin"
          className="inline-flex items-center justify-center border border-border bg-white hover:bg-slate-50 text-ink text-xs font-bold uppercase tracking-wider px-6 py-3 transition-colors rounded-sm cursor-pointer shadow-xs h-11"
        >
          إلغاء
        </Link>
        <button
